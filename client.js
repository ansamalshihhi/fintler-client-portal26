...
function renderClientPortal() {
  ...
  document.getElementById('progress-stats').innerHTML=`
    <div class="ps-card"><div class="ps-num" style="${pct===100?'color:var(--green-600)':''}">${pct}%</div><div class="ps-lbl">Complete</div><div class="progress-bar" style="margin-top:8px"><div class="progress-fill${pct===100?' complete':''}" style="width:${pct}%"></div></div></div>
  ...
  document.getElementById('checklist-items').innerHTML=`
    ${!c.items.length?'<div class="empty">No items yet. Fintler will update this shortly.</div>':''}
...
function markSubmitted(iid) {
  ...
  syncToStorage(); renderClientPortal(); toast('Marked as submitted — Fintler will be notified');
}
...
function clientUnsubmit(iid) {
  ...
  syncToStorage(); renderClientPortal(); toast('Submission undone');
}
...
function clientAddItem() {
  ...
  c.items.push(item); inp.value=''; syncToStorage(); renderClientPortal(); toast('Document added — Fintler notified');
}

function syncToStorage() {
  const clients=loadClients();
  const idx=clients.findIndex(x=>x.id===activeClient.id);
  if(idx!==-1){clients[idx]=activeClient;saveClients(clients);}
}

document.addEventListener('keydown',function(e){
  if(e.key==='Enter'&&document.getElementById('login-screen')&&!document.getElementById('login-screen').classList.contains('hidden')){tryLogin();}
});
